/**
 * @file h2o_co2_nc10_2d_traditional.cpp
 * @brief 传统 K-value H2O-CO2-nC10 规则二维基线算例主程序。
 */
...
MPMC::FluidSystem<Indices> makeFluid()
{
    ...
    fluid.waterFormationVolumeFactor = [=](Eval pressure) {
        ...
        return Eval(referenceDensity) / density;
    };
...
struct UnifiedBinaryInitialSaturation
{
    double gas{0.0};
    double water{1.0};
};

UnifiedBinaryInitialSaturation unifiedBinaryInitialSaturation(
    const MPMC::FluidSystem<Indices> &fluid)
{
#ifdef MPMC_H2O_CO2_BINARY_INITIAL_H2O_MOLE_FRACTION
    constexpr double zw = MPMC_H2O_CO2_BINARY_INITIAL_H2O_MOLE_FRACTION;
    static_assert(zw > 0.0 && zw < 1.0,
                  "Unified H2O mole fraction must lie strictly between zero and one.");
    const std::array<double, TraditionalConfig::Fluid::N> pureCo2{1.0, 0.0};
    const double zGas = fluid.eos.compressibility(
        BenchmarkCommon::initialPressure, BenchmarkCommon::temperature,
        pureCo2, MPMC::CompositionalPhase::Gas);
    const double rhoGas = fluid.eos.molarDensity(
        BenchmarkCommon::initialPressure, BenchmarkCommon::temperature,
        pureCo2, zGas, MPMC::CompositionalPhase::Gas);
    constexpr double waterMolarMass = 0.018015268;
    const double rhoWater = MPMC::IapwsIf97WaterDensity::density(
        BenchmarkCommon::initialPressure, BenchmarkCommon::temperature) /
        waterMolarMass;
    const double sw = zw * rhoGas /
        (rhoWater * (1.0 - zw) + zw * rhoGas);
    return {1.0 - sw, sw};
#else
    (void)fluid;
    return {0.0, BenchmarkCommon::targetWaterSaturation};
#endif
}
...
#ifdef MPMC_H2O_CO2_BINARY_INITIAL_H2O_MOLE_FRACTION
                primary[Indices::Primary::liquidComposition[0]] = 1.0;
                primary[Indices::Primary::vaporComposition[0]] = 1.0;
                primary[Indices::Primary::liquidSaturation] = 0.0;
                primary[Indices::Primary::vaporSaturation] = unified.gas;
                primary[Indices::Primary::waterSaturation] = unified.water;
#else
...
int run()
{
    ...
    if (rank == 0)
        PetscPrintf(PETSC_COMM_SELF,
            "[BENCHMARK] model=traditional grid=60x20x1 PV=%.12g m3 rate=%.12g m3/s "
            "inj_max_bhp=%.12g Pa prod_bhp=%.12g Pa\n"
            "[INIT] bulk z(H2O/CO2/nC10)=%.12g/%.12g/0, S(O/G/W)=0/%.12g/%.12g\n",
            BenchmarkCommon::poreVolume, BenchmarkCommon::injectionRate,
            BenchmarkCommon::injectorMaximumBhp, BenchmarkCommon::producerBhp,
            MPMC_H2O_CO2_BINARY_INITIAL_H2O_MOLE_FRACTION,
            1.0 - MPMC_H2O_CO2_BINARY_INITIAL_H2O_MOLE_FRACTION,
            unified.gas, unified.water);
#else
    if (rank == 0)
        PetscPrintf(PETSC_COMM_SELF,
            "[BENCHMARK] model=traditional grid=60x20x1 PV=%.12g m3 rate=%.12g m3/s "
    ...
    initializeSolution(grid, solution, fluid);
#ifdef MPMC_H2O_CO2_BINARY_INITIAL_H2O_MOLE_FRACTION
    runtime.initializePhaseStateFromSolution(solution, MPMC::HydrocarbonPhaseState::VaporOnly);
#else
