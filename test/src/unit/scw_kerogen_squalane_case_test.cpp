/**
 * @file scw_kerogen_squalane_case_test.cpp
 * @brief 验证超临界水驱单重质拟组分算例的配置与初始闪蒸。
 */
#include "../../../case/scw_kerogen_squalane_1d/case_config.hpp"
#include "../../../case/scw_kerogen_squalane_1d/well_config.hpp"
...
int main()
{
    try
    {
        ...
        require(CaseConfig::InitialState::pressure >
                    ScwKerogen1D::waterCriticalPressure,
                "pressure must be supercritical for water");
        ...
        require(result.converged, "initial H2O/squalane flash must converge");
        require(result.presence.count() == 1 &&
                    result.presence.contains(MPMC::CompositionalPhase::Oil),
                "experimental-endpoint baseline must start oil-rich and single phase");
        ...
        std::cout << "[PASS] scw_kerogen_squalane_case_test\n";
        return 0;
    }
    catch (const std::exception &error)
    {
        std::cerr << "[FAIL] scw_kerogen_squalane_case_test: "
                  << error.what() << '\n';
        return 1;
    }
}
