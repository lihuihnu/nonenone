/**
 * @file adaptive_structured_matrix.cpp
 * @brief 编译矩阵测试：验证 `adaptive_structured_matrix` 所覆盖模板/后端组合能够实例化。
 */
...
template <class Indices>
using Runtime = MPMC::NaturalStructuredGridRuntime<
    Indices,
    Grid<Indices>>;
...
using BaseBackend =
    MPMC::NaturalAdaptiveBackend<Runtime<BaseIndices>>;
using DissolutionBackend =
    MPMC::NaturalAdaptiveBackend<Runtime<DissolutionIndices>>;
using LandBackend =
    MPMC::NaturalAdaptiveBackend<Runtime<LandIndices>>;
using AdsorptionBackend =
    MPMC::NaturalAdaptiveBackend<Runtime<AdsorptionIndices>>;
